...
do_strip_keep_mini_debug_info() {
    ...
    if strip --strip-all -R .comment "${infile}" -o "${outfile}.tmp"; then
        ...
        objcopy -S --remove-section .gdb_index --remove-section .comment --keep-symbols="${outfile}.keep_symbols" "${outfile}.mini_debuginfo"
        objcopy --rename-section saved_debug_frame=.debug_frame "${outfile}.mini_debuginfo"
        "${XZ}" "${outfile}.mini_debuginfo"
        objcopy --add-section .gnu_debugdata="${outfile}.mini_debuginfo.xz" "${outfile}.tmp"
    else
...
do_add_gnu_debuglink() {
    objcopy --add-gnu-debuglink="${infile}" "${outfile}.tmp"
}
