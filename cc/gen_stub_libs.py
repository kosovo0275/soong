...
class SymbolFileParser(object):
        ...
        while self.next_line() != '':
            if '}' in self.current_line:
                ...
                if not base.endswith(';'):
                    raise ParseError('Unterminated version/export "C++" block (expected ;).')
                if cpp_symbols:
                    cpp_symbols = False
                else:
    ...
    def parse_symbol(self):
        ...
        if ';' not in self.current_line:
            raise ParseError('Expected ; to terminate symbol: ' + self.current_line)
        if '*' in self.current_line:
            raise ParseError('Wildcard global symbols are not permitted.')
...
def main():
    ...
    with open(args.stub_src, 'w') as src_file:
        with open(args.version_script, 'w') as version_file:
            generator = Generator(src_file, version_file, args.arch, api, args.vndk)
