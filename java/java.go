...
func (j *Module) collectBuilderFlags(ctx android.ModuleContext, deps deps) javaBuilderFlags {
	...
	} else if sdk <= 26 || !ctx.Config().TargetOpenJDK9() {
		flags.javaVersion = "1.8"
	} else if String(j.deviceProperties.Sdk_version) != "" && sdk == android.FutureApiLevel {
		flags.javaVersion = "1.8"
	} else {
	...
	if len(flags.bootClasspath) == 0 && !ctx.Config().TargetOpenJDK9() && !Bool(j.properties.No_standard_libs) && inList(flags.javaVersion, []string{"1.6", "1.7", "1.8"}) {
		java8Home := ctx.Config().Getenv("ANDROID_JAVA8_HOME")
		flags.bootClasspath = append(flags.bootClasspath,
			android.StringToPath(java8Home, "lib/jce.jar"),
			android.StringToPath(java8Home, "lib/rt.jar"))
		if Bool(j.properties.Use_tools_jar) {
			flags.bootClasspath = append(flags.bootClasspath,
				android.StringToPath(java8Home, "lib/tools.jar"))
		}
	}
